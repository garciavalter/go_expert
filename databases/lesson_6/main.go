...
func main() {
	db, err := sql.Open("mysql", "root:root@tcp(172.31.176.1:3306)/test")
	if err != nil {
		panic(err)
	}
	defer db.Close()
	...
	fmt.Printf("O produto: %s possui o preço de %.2f", p.Name, p.Price)
...
func SelectOneProduct(db *sql.DB, id string) (*Product, error) {
	...
	defer stmt.Close()
	var p Product
	//é possível usar querys com contexto usando stmt.QueryRowWithContext()
